"""
run_libero_eval_reason_vla_patched.py
...
Usage:
    python run_libero_eval_reason_vla_patched.py \
...
def main():
    parser = argparse.ArgumentParser(description="LIBERO-PRO eval for ReasonVLA (reason_vla.py)")
    ...
    # Step 2: Handle the case when only one use_xxx flag is True
        ...
        bddl_file_path = evaluation_cfg["bddl_files_path"]  # already set to bddl_base_path/task_suite_name
    ...
    # else: no perturbation flags True — use original suite as-is
    else:
    ...
    # Initialize local logging (directory path is set by shell script via --local_log_dir)
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        # The BDDL (:language ...) field is the authoritative instruction — it reflects
        # perturbations for LIBERO-PRO and is always more accurate than the filename.
        bddl_language = getattr(env, "language_instruction", None)
        # language_instruction can be a list of tokens (from BDDL parser) — normalize to string
        ...
        if bddl_language and bddl_language.strip():
            if bddl_language.strip().lower() != task_description.strip().lower():
                ...
                task_description = bddl_language.strip()  # Model receives the BDDL instruction
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    # (0 = close, 1 = open), so flip it back (-1 = open, +1 = close) before executing
