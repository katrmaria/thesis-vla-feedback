"""
reason_vla_projector_crossattn.py

Two-pass grounding using cross-attention at the projector output.

Pass 1: Full vla.forward(output_hidden_states=True) -> extract text token hidden states
        at layer 12 (default), detach.
Pass 2: vision_backbone + projector -> cross-attention(Q=projected, K,V=text_hidden)
        -> override both -> vla.forward() with labels -> loss.

Architecture rationale:
    The cross-attention operates in d_llm (4096) space, where both the projected patches
    and the text hidden states live. Single module, no per-backbone split, no dimension
    mismatch. The module is identity at init (gate = 0), so step-zero behavior equals
    the unpatched checkpoint.

    Pass 2 uses two context-manager overrides to avoid recomputing the ViT and the projector:
      - override_vision_backbone: returns cached patch_features
      - override_projector: returns the cross-attended refined patches
    Inside vla.forward(), both calls short-circuit to cached tensors.

    Stage 1: Freeze ViT + LLM, train only cross-attention module.
    Stage 2: Freeze ViT, unfreeze LLM with LoRA, train jointly.
             LoRA disabled in pass 2 so the conditioning is the only new output path.
"""
...
def main():
    ...
    if accelerator.is_main_process:
        wandb.init(project="openvla_projector_crossattn", config=vars(args), name=run_name,
